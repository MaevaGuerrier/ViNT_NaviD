...
class PathGuide:
    ...
    def __init__(self, device, ACTION_STATS, guide_cfgs=None):
        ...
        self.model.load_state_dict(torch.load(os.path.join(package_path, f'/workspace/Depth-Anything-V2/checkpoints/depth_anything_v2_{encoder}.pth'), map_location='cpu'))
    ...
    def collision_cost(self, trajs, scale_factor=None):
        ...
        print(f"cost map shape: {self.cost_map.shape}, norm_inds shape: {norm_inds.shape}")
        cost_grid = self.cost_map.T.expand(trajs_ori.shape[0], 1, -1, -1)
        print(f"cost grid shape (costmap after expand): {cost_grid.shape}")
